...
from src.fl.scaffold import (
    control_variate_bytes,
    predict_split_scaffold,
    state_l2_norm,
    sum_control_variate_states,
    train_scaffold_client,
    update_server_control_variate,
    zero_control_variate_like,
)
...
class Cluster3ProposedRunResult:
    experiment_id: str
    cluster_id: int
    dataset: str
    output_dir: Path
...
def _load_cluster3_proposed_entry(config_path: str | Path) -> tuple[Path, Mapping[str, Any], Mapping[str, Any]]:
    ...
    for cluster_entry in clusters:
        if not isinstance(cluster_entry, Mapping):
            continue
        if str(cluster_entry.get("experiment_id")) != "P_C3":
            continue
        if str(cluster_entry.get("model_family")) != "cnn1d":
            raise ValueError("P_C3 must use model_family=cnn1d.")
        if str(cluster_entry.get("fl_method")) != "SCAFFOLD":
            raise ValueError("P_C3 must use fl_method=SCAFFOLD.")
        if str(cluster_entry.get("aggregation")) != "weighted_arithmetic_mean":
            raise ValueError("P_C3 must use aggregation=weighted_arithmetic_mean.")
        return config_path, config, cluster_entry

    raise ValueError(f"{config_path}: could not find P_C3 entry in proposed config.")


def _evaluate_cluster3_split(
    ...
    *,
    ...
    for client in clients:
        ...
        split_probabilities, _ = predict_split_scaffold(state, model_config, split)
...
def run_cluster3_proposed(
    ...
    *,
...
) -> Cluster3ProposedRunResult:
    resolved_proposed_config_path, proposed_config, cluster_entry = _load_cluster3_proposed_entry(proposed_config_path)
    ...
    if cluster_id != 3:
        raise ValueError("run_cluster3_proposed only supports Cluster 3.")
    ...
    if data_summary["input_adapter"] != "feature_vector_as_sequence":
        raise ValueError("Cluster 3 proposed path requires feature-vector CNN inputs.")
    ...
    global_model = CNN1DClassifier(model_config, seed=configured_seed)
    global_state = global_model.state_dict()
    model_parameter_bytes = global_model.parameter_bytes()
    server_control_variate = zero_control_variate_like(global_state)
    client_control_variates = {
        client.client_id: zero_control_variate_like(global_state)
        for client in clients
    }
    control_state_bytes = control_variate_bytes(server_control_variate)

    output_root = Path(output_root)
    run_dir = output_root / "runs" / "P_C3"
    run_dir.mkdir(parents=True, exist_ok=True)
    metrics_csv_path = output_root / "metrics" / "P_C3_metrics.csv"
    ...
    best_subcluster_forwarded_increment_norms: Mapping[str, float] | None = None
    ...
    for round_index in range(1, configured_rounds + 1):
        ...
        forwarded_delta_sums: list[Mapping[str, np.ndarray]] = []
        forwarded_increment_norms: dict[str, float] = {}

        for subcluster_index, subcluster in enumerate(membership.subclusters):
            local_updates: list[WeightedState] = []
            local_losses: list[float] = []
            delta_control_variates: list[Mapping[str, np.ndarray]] = []
            ...
            for client_index, client in enumerate(subcluster_clients):
                result = train_scaffold_client(
                    ...
                    client_control_variates[client.client_id],
                    model_config,
                ...
                client_control_variates[client.client_id] = {
                    ...
                    for key, value in result.updated_control_variate.items()
                }
                local_updates.append(result.to_weighted_state(cluster_id=cluster_id))
                local_losses.append(result.train_loss)
                delta_control_variates.append(result.delta_control_variate)

            aggregated_subcluster_state = aggregate_leaf_updates_to_subcluster(
                local_updates,
                expected_cluster_id=cluster_id,
            )
            forwarded_delta_sum = sum_control_variate_states(delta_control_variates)
            forwarded_delta_sums.append(forwarded_delta_sum)
            forwarded_increment_norms[subcluster.subcluster_id] = state_l2_norm(forwarded_delta_sum)
        ...
        server_control_variate = update_server_control_variate(
            server_control_variate,
            forwarded_delta_sums,
            total_leaf_clients=len(clients),
        )

        train_metrics = _evaluate_cluster3_split(clients, state=global_state, model_config=model_config, split_name="train")
        validation_metrics = _evaluate_cluster3_split(
            clients,
            state=global_state,
            model_config=model_config,
            split_name="validation",
        )
        test_metrics = _evaluate_cluster3_split(clients, state=global_state, model_config=model_config, split_name="test")

        model_communication_cost_bytes = int(model_parameter_bytes * 2 * (len(clients) + membership.n_subclusters))
        control_variate_communication_cost_bytes = int(
            control_state_bytes * 2 * (len(clients) + membership.n_subclusters)
        )
        total_communication_cost_bytes = int(
            model_communication_cost_bytes + control_variate_communication_cost_bytes
        )
        ...
        round_row["control_variate_communication_cost_bytes"] = control_variate_communication_cost_bytes
        round_row["server_control_variate_l2_norm"] = state_l2_norm(server_control_variate)
        ...
        if validation_f1 is not None and float(validation_f1) >= best_validation_f1:
            ...
            best_subcluster_forwarded_increment_norms = dict(forwarded_increment_norms)

    elapsed_seconds = float(time.perf_counter() - started_at)
    if best_test_metrics is None or best_validation_metrics is None or best_train_metrics is None:
        raise ValueError("P_C3: unable to determine best validation round.")
    if best_subcluster_sample_counts is None or best_subcluster_client_counts is None:
        raise ValueError("P_C3: missing best-round subcluster statistics.")
    if best_subcluster_forwarded_increment_norms is None:
        raise ValueError("P_C3: missing forwarded SCAFFOLD increment norms.")
    ...
    if membership_contents_before != membership_contents_after:
        raise ValueError("P_C3: frozen membership file changed during proposed Cluster 3 execution.")

    final_server_control_variate_norm = state_l2_norm(server_control_variate)
    nonzero_client_control_variates = sum(
        1 for state in client_control_variates.values() if state_l2_norm(state) > 0.0
    )
    total_model_communication_cost_bytes = int(
        sum(row["communication_cost_bytes"] - row["control_variate_communication_cost_bytes"] for row in round_rows)
    )
    total_control_variate_communication_cost_bytes = int(
        sum(row["control_variate_communication_cost_bytes"] for row in round_rows)
    )
    ...
    summary = {
        ...
        "batch_size": configured_batch_size,
        "learning_rate": learning_rate,
        "optimizer_style": "corrected_sgd_update",
        "seed": configured_seed,
        "model_parameter_bytes": model_parameter_bytes,
        "control_variate_bytes": control_state_bytes,
        "control_variate_communication_cost_per_round_bytes": round_rows[-1]["control_variate_communication_cost_bytes"],
        "total_control_variate_communication_cost_bytes": total_control_variate_communication_cost_bytes,
        "model_communication_cost_per_round_bytes": round_rows[-1]["communication_cost_bytes"] - round_rows[-1]["control_variate_communication_cost_bytes"],
        "total_model_communication_cost_bytes": total_model_communication_cost_bytes,
        ...
        "best_round_subcluster_forwarded_increment_l2_norms": best_subcluster_forwarded_increment_norms,
        "final_server_control_variate_l2_norm": final_server_control_variate_norm,
        "num_client_control_variates": len(client_control_variates),
        "num_nonzero_client_control_variates": nonzero_client_control_variates,
    ...
    return Cluster3ProposedRunResult(
        experiment_id="P_C3",
        cluster_id=cluster_id,
        dataset=str(cluster_section["dataset_name"]),
        output_dir=run_dir,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the proposed Cluster 3 WUSTL + 1D-CNN + SCAFFOLD experiment."
    )
    parser.add_argument(
        "--proposed-config",
    ...
    parser.add_argument("--learning-rate", type=float, default=0.01, help="Local corrected-SGD learning rate.")
