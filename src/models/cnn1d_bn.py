...
class CNN1DBNConfig:
    ...
    def validate(self) -> None:
        if self.input_channels <= 0:
            raise ValueError("CNN1DBNConfig.input_channels must be positive.")
        if self.input_length <= 0:
            raise ValueError("CNN1DBNConfig.input_length must be positive.")
        if len(self.block_channels) != 3:
            raise ValueError("CNN1DBNConfig requires exactly three CNN blocks.")
        if len(self.kernel_sizes) != 3:
            raise ValueError("CNN1DBNConfig requires exactly three kernel sizes.")
        if any(channel <= 0 for channel in self.block_channels):
            raise ValueError("CNN1DBNConfig.block_channels values must be positive.")
        if any(kernel_size <= 0 or kernel_size % 2 == 0 for kernel_size in self.kernel_sizes):
            raise ValueError("CNN1DBNConfig.kernel_sizes values must be positive odd integers.")
        if self.hidden_dim <= 0:
            raise ValueError("CNN1DBNConfig.hidden_dim must be positive.")
        if not 0.0 <= self.dropout < 1.0:
            raise ValueError("CNN1DBNConfig.dropout must be in [0, 1).")


def _state_keys(config: CNN1DBNConfig) -> tuple[str, ...]:
    keys: list[str] = []
    for block_index, _ in enumerate(config.block_channels, start=1):
...
class CNN1DBNClassifier:
    def __init__(self, config: CNN1DBNConfig, *, seed: int = 42) -> None:
        self.config = config
    ...
    def parameter_shapes(self) -> dict[str, tuple[int, ...]]:
        ...
        for block_index, (out_channels, kernel_size) in enumerate(
            zip(self.config.block_channels, self.config.kernel_sizes),
            start=1,
        ):
            prefix = f"block{block_index}"
            shapes[f"{prefix}_conv_weight"] = (out_channels, in_channels, kernel_size)
    ...
    def load_state_dict(self, state: Mapping[str, Any]) -> None:
        ...
        if missing or extra:
            raise ValueError(
                f"CNN1DBNClassifier state keys mismatch. Missing={missing or '[]'} Extra={extra or '[]'}."
            )
        ...
        for key in self.state_keys:
            ...
            if value.shape != shapes[key]:
                raise ValueError(
                    f"CNN1DBNClassifier state {key!r} has shape {value.shape}, expected {shapes[key]}."
                )
    ...
    def train_epoch(
        ...
        *,
        ...
        if inputs.shape[0] == 0:
            raise ValueError("CNN1DBNClassifier cannot train on an empty batch.")
    ...
    def _forward(
        ...
        *,
        ...
        if inputs.ndim != 3:
            raise ValueError(
                f"CNN1DBNClassifier expects inputs with shape (batch, channels, length). Observed {inputs.shape}."
            )
        if inputs.shape[1] != self.config.input_channels or inputs.shape[2] != self.config.input_length:
            raise ValueError(
                "CNN1DBNClassifier input shape mismatch. "
        ...
        for block_index in range(1, 4):
            ...
            conv_out, conv_cache = self._conv1d_forward(prefix, x)
    ...
    def _conv1d_forward(self, prefix: str, inputs: np.ndarray) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    def _conv1d_backward(
        ...
        for kernel_index in range(kernel_size):
            ...
            grad_padded[:, :, kernel_index : kernel_index + input_length] += grad_slice
    ...
    def _dropout_forward(
        ...
        *,
        ...
        if rng is None:
            raise ValueError("CNN1DBNClassifier dropout requires an RNG during training.")
